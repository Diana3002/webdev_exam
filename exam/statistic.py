...
bp_statistic = Blueprint('statistics', __name__, url_prefix='/statistics')
PER_PAGE =10
FIELDS = ["id", "user_id", "path"]

@bp_statistic.route('/show-all')
@login_required
def show():
...
@bp_statistic.route('/show-path-user')
@check_perm('show_log')
@login_required
def show_path_user():
    cursor = db.connection().cursor(named_tuple=True)
...
@bp_statistic.route('/show-path-site')
@login_required
def show_path_site():
    cursor = db.connection().cursor(named_tuple=True)
    query = '''
...
def save_to_csv():
    ...
    if template == 'user':
        ...
        if current_user.is_admin():
            # Данные о всех посещениях для администратора
            cursor.execute('SELECT id, user_id, path FROM eventlist')
        else:
            # Данные о посещениях только для текущего пользователя
            cursor.execute('SELECT id, user_id, path FROM eventlist WHERE user_id = %s', (current_user.id,))
        ...
        # Данные по пользователям (в данном случае только для текущего пользователя)
        cursor.execute('SELECT COUNT(*) as count, user_id FROM eventlist WHERE user_id = %s GROUP BY user_id', (current_user.id,))
        ...
        # Данные по страницам (в данном случае для всех)
    ...
    return send_file(
        ...
        as_attachment=True, #файл должен быть отправлен как вложение, а не как часть страницы.
        mimetype='text/csv'
    )
